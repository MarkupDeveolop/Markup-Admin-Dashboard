...
import { BrandSchema } from "@/validations/brands/brands";

type ExportServiceFormValues = z.infer<typeof BrandSchema>;

interface ExportServiceFormProps {
  initialData: ExportServiceFormValues | null;
}

const BrandForm: React.FC<ExportServiceFormProps> = ({ initialData }) => {
  ...
  const description = initialData
    ? "Edit the selected Brand details."
    : "Add a new export service entry.";
  const toastMessage = initialData
    ? "Brand updated successfully."
    : "Brand created successfully.";
  const action = initialData ? "Save Changes" : "Create Brand";

  const form = useForm<ExportServiceFormValues>({
  ...
  const onSubmit = async (data: ExportServiceFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`${DOMAIN}/api/brands/brand/${params.slug}`, data);
      } else {
        await axios.post(`${DOMAIN}/api/brands/brand`, data);
      }
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`${DOMAIN}/api/brands/brand/${params.slug}`);
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <div className="">
                  <FormField
                    control={form.control}
                    name={`brandFeature.${index}.imageUrl`}
                    render={({ field }) => (
                      <FormItem>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    control={form.control}
                    name={`brandFeature.${index}.titleEn`}
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>title (English)</FormLabel>
                        <FormControl>
                  ...
                  <FormField
                    control={form.control}
                    name={`brandFeature.${index}.titleAr`}
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>title (Arabic)</FormLabel>
                        <FormControl>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    control={form.control}
                    name={`brandFeature.${index}.featureEn`}
                    render={({ field }) => (
                      <FormItem>
                  ...
                  <FormField
                    control={form.control}
                    name={`brandFeature.${index}.featureAr`}
                    render={({ field }) => (
                      <FormItem>
